...
String LED_num,Light_num,KEEP,Choose_light;
int led_num,light_num,keep=0,choose_light;
/*
 * LED_num        幾顆LED同時動作
 * Light_num      LED亮度
 * Breathe_speed  呼吸速度
...
void setup() {
  ...
  keep=EEPROM.read(6);
  if(keep==0){while (WiFi.status() != WL_CONNECTED) {delay(1);}}
